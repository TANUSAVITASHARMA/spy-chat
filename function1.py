...
def select_a_friend():
    ...
    for friend in friends:
        print ('%d. %s %s age %d with rating %.2f'  % (item_number, friend.name,friend.salutation,int(friend.age),float(friend.rating)))
     ...
     for chat in friends[read_for].chats:
      if chat.sent_by_me:
        print(Fore.BLUE+ '[%s] %s: %s' % (chat.time.strftime("%d %B %Y"), 'You said:', chat.message))
         ...
         print (Fore.CYAN+'[%s] %s said: %s' % (chat.time.strftime("%d %B %Y"), friends[read_for].name, chat.message))
